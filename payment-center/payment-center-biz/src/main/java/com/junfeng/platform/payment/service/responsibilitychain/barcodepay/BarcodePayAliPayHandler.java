...
import com.junfeng.platform.payment.bank.jxbank.request.JxBankAliPayBarcodeRequest;
 ...
 * @author:xionghui
 * @date:2018年8月20日 下午3:55:17
 * @version 1.0
 */
@Component
public class BarcodePayAliPayHandler extends AbstractHandler<BarcodePayHandleParams> {
    ...
    public BarcodePayHandleParams handleRequest(BarcodePayHandleParams requestParam) throws Exception {
        ...
        if (StringUtils.equals(requestParam.getPayChannelCode(), PayChannelCode.JXBANK.getValue())) {
            if (StringUtils.equals(requestParam.getPaymentModeCode(), PaymentModeCode.ALIPAY_BARCODE.getValue())) {
                ...
                JxBankAliPayBarcodeRequest request = new JxBankAliPayBarcodeRequest(jxBankBarcodeParam);
        ...
        if (StringUtils.equals(PayChannelCode.CHINABANK.getValue(), requestParam.getPayChannelCode())) {
            // 支付宝
            if (StringUtils.equals(PaymentModeCode.ALIPAY_BARCODE.getValue(), requestParam.getPaymentModeCode())) {
